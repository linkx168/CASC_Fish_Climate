...
halk_aged_by_spp_path <- "data/parquet_files/il_data/halk_aged_data_by_spp/"
...
combine_aged_files <- lapply(age_structures, function(x) {
  ...
  halk_aged_datasets <- 
    open_dataset(here::here(halk_aged_files))
    
  write_dataset(
    halk_aged_datasets, 
    path = here::here(sprintf(
      "data/parquet_files/il_data/halk_aged_data/%ss",
      x
    ))
  )
})
