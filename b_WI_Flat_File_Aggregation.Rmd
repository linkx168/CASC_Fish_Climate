...
#UPDATE FILE PATH TO IL DISAGGREGATED DATA FOLDER
...
wi_target_cisco8nov2023 <- read_csv()
...
#update file path for mglp fish lakes crosswalk
MGLP_crosswalk <- fread()
...
#update filepath for wi_lake_wbic_lat_long_PNF.csv
wi_lat_long <- read_csv()%>%
...
#update filepath for 24k_Hydro_Waterbodies_(Open_Water).csv
WI_hydro_list <- read_csv()
...
#here we read in a gear cross walk, the file contains each unique gear and assigns it new labels that are consistent across states
  #update file path for gears_by_state.csv
gear_xwalk <- fread(, na.strings = "")%>% 
...
# Adding HALK Ages
```{r}
#update filepath for halked_fish_xwalk.csv
...
#update with filepath for saving
fwrite(wi_inland_fishobservations_10Nov2023,  , na="NA" )

#remove the object
rm(wi_inland_fishobservations_10Nov2023)

#read in the object
#update with filepath where .csv was saved
wi_dat <- fread(, na.strings = "NA")
glimpse(wi_dat)
...
write_dataset(dataset = wi_dat, 
              path = , 
              format = "parquet")
#update with filepath were parquet was saved
wi_data <- open_dataset()
glimpse(wi_data)
