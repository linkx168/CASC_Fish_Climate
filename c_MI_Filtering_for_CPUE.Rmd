...
mi_data <- open_dataset(sources = file.path(), partitioning = c("state")) %>% 
...
#update with filepath to inclusion/filtering table
...
```{r}
#filters for metric (adult abundance) and selects columns needed for survey filtering#update with filepath to inclusion/filtering table
filter_table_effort <- read_csv(file.path()) %>% 
...
#update with filepath for saving state specific abudance file
#write_csv(mi_filtered_species, ".csv")
```
