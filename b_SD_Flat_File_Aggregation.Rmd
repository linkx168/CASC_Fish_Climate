...
#UPDATE FILE PATH TO sd DISAGGREGATED DATA FOLDER
...
#update filepath for SD_gear_unitizer.csv
gear_units <- fread()
...
#update with filepath for mglp fish lakes 
crosswalk_msu <- read_csv() %>% 
  filter(STATE == "SD") %>% 
...
#update filepath for lake metadata
lat_lon_usgs <- read_csv() %>% 
  filter(state == "SD") %>%
  select(site_id, 
         centroid_lat,
         centroid_lon) %>% 
...
#update filepath for gears_by_state.csv
gear_xwalk <- read_csv() %>% 
...
# Adding HALK ages
```{r}
#update with file path for "halked_fish_xwalk.csv"
...
write_dataset(dataset = sd_merged_ce_trial, path = )

#update with saved filepath of parquet file
sd_data <- open_dataset(sources = )
