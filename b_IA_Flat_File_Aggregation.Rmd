...
#UPDATE FILE PATH TO IOWA DISAGGREGATED DATA FOLDER
files_list <- list.files(path = , pattern = ".+\\.csv") #grabs only.csv files
files_list
...
#SET FILE PATH TO IA_gear_notes.csv, CONTACT AUTHORS FOR THIS FILE
gear_corrections <- fread(file = )
...
#UPDATE FILE PATH TO IA_comments_notes.csv, CONTACT AUTHORS FOR FILE
    notes_corrections <- fread()
...
#UPDATE FILEPATH TO gearS_by_state.csv
gear_xwalk <- read_csv() %>% 
  filter(state == "Iowa") %>% 
...
#UPDATE FILE PATH TO ia_fishlengths_24Oct2023.csv FROM DISAGGREGATED FOLDER
ia_dat_old <-  fread()
...
# UPDATE FILEPATH TO MGLP_FISH_LAKES_12Aug24.csv
msu_crosswalk <- read_csv() %>% 
  filter(STATE == "IA") %>% 
  select(NHDHR_ID,
         STATE_ID,
         LAT,
...
#UPDATE FILEPATH TO lake_metadata.csv
usgs <- read_csv() %>% 
  filter(state == "IA") %>% 
  select(site_id, 
         lake_name,
...
# Adding HALK Ages
```{r}
#UPDATE FILEPATH TO HALKED FISH CROSSWALK (halked_fish_xwalk.CSV), created in b_HALK
halk <- read_csv(file.path()) 
...
# UPDATE FILEPATH FOR SAVING
write_dataset(dataset = ia_catch_eff, path = )

#UPDATE FILE PATH FROM WHERE THE PARQUET FILEPATH WAS SAVED
ia_data <- open_dataset()
...
#UPDATE FILEPATH TO IA PARQUET FILE
