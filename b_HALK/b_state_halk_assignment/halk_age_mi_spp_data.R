...
tictoc::tic()
mi_halk_aged_data <- lapply(spp_list, function(x) {
    formatted_data <- 
      mi_data |> 
      ...
      mutate(
        ...
        length.unit = case_when(!is.na(length) ~ "in",
                                !is.na(length.bin) ~ "in",
    ...
    halk_str_aged_data <- lapply(age_structures, function(y, spp = x) {
      ...
      file_path <- paste0(
        "data/parquet_files/mi_data/halk_aged_data_by_spp/", 
        spp,
        "/", paste0(y, "s")
      )
