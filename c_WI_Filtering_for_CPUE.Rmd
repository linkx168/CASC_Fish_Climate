...
# Data
```{r}
#update filepath for hived data set
wi_data <- open_dataset(sources = file.path(), partitioning = c("state")) %>% 
...
#filters for metric (adult abundance) and selects columns needed for survey filtering
#update file path for inclusion/filtering table
filter_table_effort.standard <- read_csv(file.path()) %>% 
...
#update file path for inclusion/filtering table
filter_table_effort.extra <- read_csv(file.path()) %>% 
  filter(metric == "adult_abundance" & state == "Wisconsin" & species_1 == "largemouth_bass") %>%
  select(-area_group,
         -metric,
         -species_1,
...
#update with state specific abundance saving file path
#write_csv(wi_filtered_species, ".csv")
```
